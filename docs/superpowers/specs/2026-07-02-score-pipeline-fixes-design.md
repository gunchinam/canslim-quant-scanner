...
## Phase D — 종가베팅 판단(leading signal) 개선

### 현상 (`web_app/macro.py:253-315` `_leading_signal`)
- VIX 텀스트럭처 + SKEW + HY 스프레드 3개로 safe/caution/danger 판정.
- **치명적 결함**: `_fetch_yf`가 일봉 종가만 사용 → 한국장 오후 종가베팅 시점(15:00~15:30 KST)에 보는 VIX/SKEW/HYG는 **전날 미국 종가(~17시간 전)**. 오늘 밤 미국을 예고하는 실시간 신호(미 지수 선물)가 전혀 없음.
- USDKRW·KOSPI를 수집하면서 종가베팅 판단에는 미사용.
- SKEW 지수는 실증적으로 꼬리위험 예측력이 없다는 평가가 지배적(공신력 부족) — 반면 VIX 백워데이션·HY 스프레드는 근거가 탄탄.

### 설계: market-aware 판정
`_leading_signal(yf_data, market)` 로 확장, 스캐너의 시장 탭(KR/US)에 따라 지표 세트를 분기. `/api/macro`에 `market` 쿼리 파라미터 추가(기본 KR), 프론트는 현재 탭을 전달.

**공통 준비**: `_YF_MAP`에 미 지수 선물 `ES=F`(sp_fut), `NQ=F`(nq_fut) 추가. 선물은 24시간 거래라 yfinance 일봉의 마지막(진행 중) bar가 근실시간 값 — 별도 인프라 없이 신선한 신호 확보. KOSPI/USDKRW도 한국 장중엔 당일 진행 bar라 신선.

**KR 모드 (한국장 종가베팅)** — danger 가중치:
| 지표 | 주의(+1) | 위험(+2) | 신선도 |
|---|---|---|---|
| S&P 선물 당일 변화 | ≤ −0.5% | ≤ −1.0% | 실시간 |
| USDKRW 당일 변화 | ≥ +0.5% | ≥ +1.0% | 실시간 |
| KOSPI 당일 변화 | ≤ −1.0% | ≤ −2.0% | 실시간 |
| VIX 텀스트럭처 | > 1.0 | > 1.05 | 전일(구조 신호라 유효) |
| HY 스프레드(HYG−LQD) | < −0.3%p | < −1.0%p | 전일 |

**US 모드 (미국장 종가베팅)**: VIX 텀스트럭처·HY 스프레드(장중이라 신선), S&P 당일 변화(≤−1%/−2%), 10Y 금리 당일 급등(기존 `_US10Y_STRESS` 재사용). 선물·원화 항목 제외.

**공통 규칙**:
- SKEW는 판정에서 제거(표시용 참고값으로만 유지).
- 판정 임계 유지: danger_count ≥3 → danger, ≥1 → caution, 0 → safe.
- `reasons`에 각 지표의 신선도 표기(예: "전일 종가 기준") — 사용자가 판단 근거의 시점을 알 수 있게.
- 반환에 `market`, `as_of` 필드 추가.

### 검증
- 단위 테스트: 고정 yf_data 입력 → KR/US 모드별 기대 판정(모킹, 네트워크 불요).
- 판정 히스토리 JSONL(`cache_v19/leading_signal_log.jsonl`, 판정 시각·지표·판정) 적재 — 추후 다음날 갭과 대조해 임계 재조정 근거로 사용(판독은 스코프 밖).
