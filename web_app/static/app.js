...
// ── 드로어 종합 판정 카드 (Verdict Card) ──────────────────────────────────
// _composeVerdict: 밸류에이션·재무건전성·타이밍 리스크 3개 신호를 규칙 기반으로
// 합성하는 순수 함수. DOM 접근 없음 — node 로 직접 extract·evaluate 가능해야 한다
// (web_app/tests/test_verdict_card_js.py 참조). 외부 함수(esc/fmtPrice 등) 호출 금지 —
// 테스트 하네스가 이 함수 블록만 잘라내 단독 실행하기 때문.
function _composeVerdict(detail, nomura, fourAxis) {
  const d = detail || {};
  const ep = d.EntryPlan || {};
  const nm = nomura || null;
  const fa = fourAxis || null;

  // ── 축 1: 품질(quality) — detail 우선, nomura로 보강 ──
  const piotroski = (nm && nm.piotroski != null) ? nm.piotroski : (d.PiotroskiF != null ? d.PiotroskiF : null);
  const altmanZ   = (nm && nm.altman_z  != null) ? nm.altman_z  : (d.AltmanZ    != null ? d.AltmanZ    : null);
  const quantScore = (nm && nm.quantitative_score != null) ? nm.quantitative_score : null;

  let quality = null;
  if (piotroski != null || altmanZ != null) {
    const isRed = (altmanZ != null && altmanZ < 1.81) || (piotroski != null && piotroski <= 3);
    const goodPiotroski = piotroski != null ? piotroski >= 6 : (quantScore != null ? quantScore >= 75 : false);
    const isGood = (altmanZ != null && altmanZ > 2.99) && goodPiotroski;
    quality = isRed ? 'red' : (isGood ? 'good' : 'mid');
  }
  const zTxt = altmanZ != null ? (altmanZ > 2.99 ? 'Altman 안전' : altmanZ > 1.81 ? 'Altman 회색지대' : 'Altman 위험') : 'Altman 데이터없음';
  const fTxt = piotroski != null ? `F${piotroski}` : 'F—';
  const qualityReason = quality == null ? null
    : quality === 'red'  ? `재무 위험 (${zTxt} · ${fTxt})`
    : quality === 'good' ? `재무 건전 (${zTxt} · ${fTxt})`
    : `재무 보통 (${zTxt} · ${fTxt})`;

  // ── 축 2: 밸류에이션(valuation) — nomura 필요 ──
  const ff = (nm && Array.isArray(nm.football_field)) ? nm.football_field : [];
  const nomuraUpside = (nm && nm.nomura_upside != null) ? nm.nomura_upside : null;
  const cp = (nm && nm.current_price != null) ? nm.current_price : (d.Price != null ? Number(d.Price) : null);

  let aboveCount = 0, belowCount = 0;
  let lowestAboveBand = null;   // above 밴드 중 max_price 가 가장 낮은 것 (트리거용)
  if (ff.length > 0 && cp != null) {
    for (const band of ff) {
      if (band && band.max_price != null && cp > band.max_price) {
        aboveCount++;
        if (lowestAboveBand == null || band.max_price < lowestAboveBand.max_price) lowestAboveBand = band;
      }
      if (band && band.min_price != null && cp < band.min_price) belowCount++;
    }
  }
  const n = ff.length;
  ...
  let valuation = null;
  if (n > 0 || nomuraUpside != null) {
    const isExpensive = (n > 0 && aboveCount >= half) || (aboveCount >= 1 && nomuraUpside != null && nomuraUpside < -10);
    const isCheap = (n > 0 && belowCount >= half) || (nomuraUpside != null && nomuraUpside >= 15);
    valuation = isExpensive ? 'expensive' : (isCheap ? 'cheap' : 'fair');
  }
  let valuationReason = null;
  if (valuation === 'expensive' && lowestAboveBand) {
    valuationReason = `${lowestAboveBand.method || 'DCF'} 밴드 상단 ${Math.round(lowestAboveBand.max_price).toLocaleString()} 초과 — 고평가`;
  } else if (valuation === 'expensive') {
    valuationReason = `목표가 업사이드 ${nomuraUpside != null ? nomuraUpside.toFixed(1) : '—'}% — 고평가`;
  } else if (valuation === 'cheap') {
    valuationReason = nomuraUpside != null
      ? `목표가 업사이드 +${nomuraUpside.toFixed(1)}% — 저평가`
      : 'Football Field 밴드 하단 미만 — 저평가';
  } else if (valuation === 'fair') {
    valuationReason = '밸류에이션 적정 구간';
  }

  // ── 축 3: 타이밍(timing) — detail의 EntryPlan + four_axis 보강 ──
  const vel5d = ep.dd_velocity_5d != null ? ep.dd_velocity_5d : null;
  const mddCurrent = ep.mdd_current != null ? ep.mdd_current : null;
  const trendScore    = (fa && fa.trend    && fa.trend.score    != null) ? fa.trend.score    : null;
  const momentumScore = (fa && fa.momentum && fa.momentum.score != null) ? fa.momentum.score : null;
  const volumeScore   = (fa && fa.volume   && fa.volume.score   != null) ? fa.volume.score   : null;

  let timing = null;
  if (vel5d != null || mddCurrent != null || fa != null) {
    const isKnife = vel5d != null && vel5d <= -5;
    const isWeak = (vel5d != null && vel5d <= -2) || (mddCurrent != null && mddCurrent <= -20) ||
                   (momentumScore != null && momentumScore <= 2) || (volumeScore != null && volumeScore <= 1);
    const isStrong = (trendScore != null && trendScore >= 4) && (momentumScore != null && momentumScore >= 4) &&
                      (vel5d == null || vel5d > -2);
    timing = isKnife ? 'knife' : (isWeak ? 'weak' : (isStrong ? 'strong' : 'ok'));
  }
  let timingReason = null;
  if (timing === 'knife') {
    timingReason = `5일간 ${vel5d.toFixed(1)}%p 급락 진행 중`;
  } else if (timing === 'weak') {
    if (vel5d != null && vel5d <= -2) timingReason = `5일간 ${vel5d.toFixed(1)}%p 낙폭 진행 중`;
    else if (mddCurrent != null && mddCurrent <= -20) timingReason = `현재 고점 대비 ${mddCurrent.toFixed(1)}% 낙폭 (MDD)`;
    else if (momentumScore != null && momentumScore <= 2) timingReason = `4축 모멘텀 약화 (${momentumScore}점)`;
    else timingReason = `4축 거래량 약세 (${volumeScore}점)`;
  } else if (timing === 'strong') {
    timingReason = `추세·모멘텀 강함 (트렌드 ${trendScore} · 모멘텀 ${momentumScore})`;
  } else if (timing === 'ok') {
    timingReason = '타이밍 특이사항 없음';
  }

  // ── 판정 매트릭스 (first-match) ──
  // 1~3: 단일 축 긴급 신호는 데이터 부족 여부와 무관하게 즉시 우선 적용.
  // 데이터가 부족(유효 축 ≤1개)하면 그 다음으로 HOLD — 4~6(BUY/SPLIT/WAIT-weak)
  // 판정은 최소 2개 이상의 축이 확보됐을 때만 의미가 있으므로 HOLD 체크를 그 앞에 둔다.
  const validAxisCount = [quality, valuation, timing].filter(x => x != null).length;
  let verdict, label;
  if (quality === 'red') {
    verdict = 'AVOID'; label = '회피 — 재무 위험 신호';
  } else if (timing === 'knife') {
    verdict = 'WAIT'; label = '관망 — 급락 진행 중 (추격 금지)';
  } else if (valuation === 'expensive') {
    verdict = 'WAIT'; label = '관망 — 밸류에이션 부담';
  } else if (validAxisCount <= 1) {
    verdict = 'HOLD'; label = '판단 보류 — 데이터 수집 중';
  } else if (quality === 'good' && valuation === 'cheap' && timing === 'strong') {
    verdict = 'BUY'; label = '매수 적기';
  } else if (timing === 'weak') {
    verdict = 'WAIT'; label = '관망 — 수급/모멘텀 약함';
  } else {
    verdict = 'SPLIT'; label = '분할 접근 — 아래 분할매수 플랜 참조';
  }

  // ── confidence ──
  const confidence = validAxisCount === 3 ? '높음' : validAxisCount === 2 ? '중간' : '낮음';

  // ── reasons (축별 1줄, 최대 3줄) ──
  const reasons = [qualityReason, valuationReason, timingReason].filter(r => r != null);

  // ── triggers ──
  const triggers = [];
  if (valuation === 'expensive' && lowestAboveBand) {
    triggers.push({ text: `${Math.round(lowestAboveBand.max_price).toLocaleString()}원 (${lowestAboveBand.method || 'DCF'} 밴드 상단) 이하 진입 시 재검토` });
  } else if (timing === 'knife' || timing === 'weak') {
    triggers.push({ text: '낙폭 속도 진정(-2%p 이내) 후 재검토' });
  } else if (verdict === 'SPLIT' || verdict === 'BUY') {
    triggers.push({ text: '진입 시 하단 분할매수 플랜 참조', scrollTarget: 'dp-split-plan' });
  }
  ...
  return {
    ...
    triggers,
    confidence,
    axes: { quality, valuation, timing },
  };
}

// ── Verdict Card 렌더러 + 오케스트레이션 ─────────────────────────────────
const _VC_COLORS = { BUY: '#16A34A', SPLIT: '#3182F6', WAIT: '#F59E0B', AVOID: '#DC2626', HOLD: '#9CA3AF' };

function _renderVerdictCard(result) {
  const host = document.getElementById('dp-verdict-card');
  if (!host) return;
  if (!result) { host.innerHTML = ''; return; }
  const col = _VC_COLORS[result.verdict] || '#9CA3AF';
  const reasonsHtml = (result.reasons || []).map(r => `<li class="vc-reason">${esc(r)}</li>`).join('');
  const triggersHtml = (result.triggers || []).map(t => {
    const attr = t.scrollTarget ? ` data-vc-scroll="${esc(t.scrollTarget)}"` : '';
    return `<div class="vc-trigger"${attr}>${esc(t.text)}</div>`;
  }).join('');
  host.innerHTML = `
    <div class="vc-card" style="border-color:${col}40;">
      <div class="vc-head">
        <span class="vc-badge" style="background:${col};">${esc(result.verdict)}</span>
        <span class="vc-label">${esc(result.label)}</span>
        <span class="vc-conf">확신도 ${esc(result.confidence)}</span>
      </div>
      ${reasonsHtml ? `<ul class="vc-reasons">${reasonsHtml}</ul>` : ''}
      ${triggersHtml}
    </div>`;
  host.querySelectorAll('[data-vc-scroll]').forEach(el => {
    el.addEventListener('click', () => {
      const target = document.getElementById(el.getAttribute('data-vc-scroll'));
...
let _vcReqSeq = 0;
let _vcNomuraData = null;

// 현재 드로어 티커에 대해 이미 로드 완료된 four_axis 데이터가 있으면 반환 (중복 fetch 없이 재사용)
function _vcCurrentFourAxis() {
  const tk = _lastDetailData && _lastDetailData.Ticker;
  return (tk && _dpFourAxisLoadedFor === tk) ? _dpFourAxisData : null;
}

// 드로어가 열릴 때(= _populatePanelDetail) 호출. ① detail만으로 1차 렌더 →
// ② /api/nomura-score eager fetch 후 재렌더 (four_axis처럼 무거운 차트 렌더가
// 없는 가벼운 호출이라 verdict 전용으로 별도 fetch — four_axis는 중복 fetch 금지
// 라서 'dp-fouraxis-loaded' 이벤트 구독으로만 재사용한다).
function _startVerdictCard(d) {
  const seq = ++_vcReqSeq;
  _vcNomuraData = null;
  _renderVerdictCard(_composeVerdict(d, null, _vcCurrentFourAxis()));

  const ticker = d && d.Ticker;
  if (!ticker) return;

  fetch(`/api/nomura-score/${encodeURIComponent(ticker)}`)
    .then(res => res.json())
    .then(json => {
      if (seq !== _vcReqSeq) return;  // 티커 전환 시 stale 응답 가드
      if (json && json.status === 'ok' && json.data) {
        _vcNomuraData = json.data;
        _renderVerdictCard(_composeVerdict(_lastDetailData, _vcNomuraData, _vcCurrentFourAxis()));
      }
    })
    .catch(() => {});
}

// ③ four_axis 로드 완료 이벤트 구독 — 전역에 1회만 등록 (populate 때마다 재등록하면
// 리스너가 쌓이므로 스크립트 로드 시점에 단 한 번 붙인다).
document.addEventListener('dp-fouraxis-loaded', () => {
  if (!_lastDetailData) return;
  _renderVerdictCard(_composeVerdict(_lastDetailData, _vcNomuraData, _vcCurrentFourAxis()));
});
...
let _dpFourAxisData = null;   // four_axis 응답 캐시 (verdict card용 — 'dp-fouraxis-loaded' 이벤트로 구독)
